...
exports.Send = async function Send() {
    ...
    Letters.map(async letter => {
        if (letter.send == false) {
             console.log("타이틀" + `수진이의 편지: ${letter.title}`)
             console.log("컨텐츠" + letter.contents)
            /*
            const message = new thecamp.Message(letter.title, letter.contents, trainee);
            await thecamp.sendMessage(cookies, trainee, message);

             */
            await Private.findOneAndUpdate({ _id: letter._id }, { send: true })
            await sleep(10000)
        }
    })
}
